...
public class GjtGraduationNativeDao {
	...
	public Page<Map<String, Object>> queryGraduationSpecialtyList(Map<String, Object> searchParams, PageRequest pageRequst) {
		...
		sb.append(" select ")
			...
			.append("     gg.GRADE_ID as \"gradeId\",")
			.append("     gg.GRADE_NAME as \"gradeName\",")
			.append("     case when gg.START_DATE > sysdate then 1 when gg.START_DATE <= sysdate and gg.END_DATE >= sysdate then 2 else 3 end as \"status\",")
			...
			.append(" inner join ")
			.append("      GJT_GRADE gg ")
			.append(" on gg.GRADE_ID = b.GRADE_ID ")
			.append(" where ")
		...
		if (status != null && StringUtils.isNotBlank((String)status)) {
			if ("1".equals(status)) {  //未开始
				sb.append(" and gg.START_DATE > sysdate ");
			} else if ("2".equals(status)) {  //进行中
				sb.append(" and gg.START_DATE <= sysdate and gg.END_DATE >= sysdate ");
			} else if ("3".equals(status)) {  //已结束
				sb.append(" and gg.END_DATE < sysdate ");
			}
		}
	...
	public float queryCompulsorySumScore(String studentId) {
		...
		sb.append(" select ")
			...
			.append("      VIEW_TEACH_PLAN gtp")
			.append("      on ")
			.append("      vsi.GRADE_SPECIALTY_ID = gtp.GRADE_SPECIALTY_ID")
	...
	public float queryDegreeScore(String studentId) {
		...
		sb.append(" select ")
			...
			.append("      vsi.GRADE_SPECIALTY_ID = gtp.GRADE_SPECIALTY_ID")
			.append("      and gtp.IS_DELETED = 'N'")
			.append(" inner join GJT_COURSE gc")
			.append("      on ")
			.append("      gc.COURSE_ID=gtp.COURSE_ID")
			.append("      and gc.COURSE_CATEGORY = 3")
			.append(" left join ")
			.append("       GJT_REC_RESULT grr")
			.append("      on ")
			.append("      grr.STUDENT_ID = vsi.STUDENT_ID")
	 ...
	 * @return
	 */
	public List<Map<String, Object>> queryStudentRegisterMsgNew(Map<String, Object> searchParams) {
		Map<String, Object> map = new HashMap<String, Object>();
		StringBuffer sb = new StringBuffer();
		sb.append(" select ")
			.append("     gsi.STUDENT_ID as \"studentId\", ")
			.append("     gsi.XM as \"studentName\", ")
			.append("     gsi.XH as \"studentCode\", ")
			.append("     gsi.SFZH as \"sfzh\", ")
			.append("     nvl(gsi.ERP_REGISTRATION_NUMBER,' ') as \"erpRegistrationNumber\", ")
			.append("     nvl(to_char(to_date(gsi.CSRQ, 'YYYYMMDD'), 'YYYY\"年\"MM\"月\"DD\"日\"'), ' ') as \"birthday\", ")
			.append("     nvl(d1.NAME, ' ') as \"sex\", ")
			.append("     nvl(d2.NAME, ' ') as \"nations\", ")
			.append("     '身份证' as \"credentialsType\", ")
			...
			.append("      and d4.CODE = gsi.PYCC")
			.append(" where ")
			.append("  vsi.grade_id =:gradeId ")
			.append("  and r.submit_type ='2' ");
		map.put("gradeId", ObjectUtils.toString(searchParams.get("afterGradeId")));
		return commonDao.queryForMapList(sb.toString(), map);
	}
	/**
	 * 查询未填写和已填写毕业登记表的学员
	 * @param gradeId
	 * @return
	 */
	public List<Map<String, Object>> queryStudentRegisterProgress(String gradeId) {
		Map<String, Object> map = new HashMap<String, Object>();
		...
		sql.append("  '未提交'");
		sql.append("  end");
		sql.append("  from gjt_graduation_register ggr");
		sql.append("  where ggr.student_id = gsi.student_id");
		sql.append("  and ggr.is_deleted = 'N' and ggr.submit_type='2') progress");
