			...
			<div class="scroll-box">
				<div class="box-body">
					<div class="row">
						...
						<div class="col-xs-2 no-pad-right">
							<select name="pycc" class="form-control">
								<option value="">全部层次</option>
								<c:forEach items="${pyccMap }" var="map">
									<option value="${map.key }"  <c:if test="${param.pycc eq map.key }">selected</c:if>>${map.value }</option>
								</c:forEach>
							</select>
						</div>
						
						<div class="col-xs-2 no-pad-right">
							<select name="gradeId" class="form-control selectpicker show-tick" data-size="8"
											data-live-search="true">
								<option value="">全部年级</option>
								<c:forEach items="${gradeMap }" var="map">
									<option value="${map.key }"  <c:if test="${param.gradeId eq map.key }">selected</c:if>>${map.value }</option>
								</c:forEach>
							</select>
						</div>
						
						<div class="col-xs-2 no-pad-right">
							<select name="specialtyId" class="form-control selectpicker show-tick" data-size="8"
											data-live-search="true">
								<option value="">全部专业</option>
								<c:forEach items="${specialtyMap }" var="map">
									<option value="${map.key }"  <c:if test="${param.specialtyId eq map.key }">selected</c:if>>${map.value }</option>
								</c:forEach>
							</select>
						</div>
