						...
						<div class="col-xs-6 col-sm-4 pad-b5">
							...
							<span>
								...
								<shiro:lacksPermission name="/personal/index$privacyJurisdiction">
									<c:if test="${not empty studentMap.SJH }">
									${fn:substring(studentMap.SJH,0, 3)}******${fn:substring(studentMap.SJH,8, (studentMap.SJH).length())}
									</c:if>
			...
			<div class="row stu-info-status">
				...
				<div class="col-sm-2 col-xs-6">
					<div class="f24 text-center">
						<c:choose>
							<c:when test="${studentMap.PC_ONLINE_COUNT ne 0}">${studentMap.PC_ONLINE_PERCENT}%</c:when>
				...
				<div class="col-sm-2 col-xs-6">
					<div class="f24 text-center">
						<c:choose>
							<c:when test="${studentMap.APP_ONLINE_COUNT ne 0}">${100-studentMap.PC_ONLINE_PERCENT}%</c:when>
							<c:otherwise>0%</c:otherwise>
						</c:choose>
					</div>
				...
				<div class="col-sm-2 col-xs-6">
					<div class="f24 text-center">${studentMap.MAX_LAST_LOGIN_TIME}</div>
	...
	<div class="box box-border margin-bottom-none">
		...
		<div class="box-header with-border">
			<div class="box-header with-border">
				...
				<shiro:hasPermission name="/studymanage/getCourseClassLoginList$export">
					<div class="pull-right no-margin">
						<a href="${ctx}/studymanage/studentLoginExport/${fn:length(resultList)}" class="btn btn-default btn-sm margin_l10" data-role="export"><i class="fa fa-fw fa-sign-out"></i> 导出学习记录明细</a>
					</div>
				</shiro:hasPermission>
			</div>
			<div class="box-body">
							...
							<c:when test="${not empty resultList}">
								<c:forEach items="${resultList}" var="item">
									<tr>
										...
										<td>${item.LOGIN_TIME_COUNT}</td>
										<td>
											<c:choose>
												<c:when test="${item.LOGIN_TIMES eq 0}">--</c:when>
										...
										<td>
											<c:if test="${item.IS_ONLINE eq 'N' and item.LOGIN_TIMES > 0}"><div class="gray9">离线<br>(${item.LEFT_DAY}天未学习)</div></c:if>
											<c:if test="${item.IS_ONLINE eq 'N' and item.LOGIN_TIMES eq 0}"><div class="gray9">离线<br>(从未学习)</div></c:if>
											<c:if test="${item.IS_ONLINE eq 'Y'}"><div class="text-green">在线</div></c:if>
										</td>
										<td>
											<c:if test="${not empty item.TERMCOURSE_ID}">
												<a href="${ctx}/studymanage/courseCondition/${item.TERMCOURSE_ID}/${item.STUDENT_ID}/${item.COURSE_ID}" class="operion-item" data-toggle="tooltip" title="查看课程学情明细"><i class="fa fa-fw fa-view-more"></i></a>
											</c:if>
