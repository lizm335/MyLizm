...
<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li><a href="#">实践管理</a></li>
		<li><a href="#">实践计划</a></li>
		<li class="active">新增实践计划</li>
	</ol>
</section>
<section class="content">
	<div class="box margin-bottom-none">
		<div class="box-body">
				<form id="theform" action="${ctx }/practicePlan/${action}" method="post">
					<input id="action" type="hidden" name="action" value="${action}">
	            	<input type="hidden" name="practicePlanId" value="${entity.practicePlanId}">
								...
								<tr>
									...
									<td width="35%">
										<div class="position-relative">
											<input name="practicePlanName" value="${entity.practicePlanName}" type="text" class="form-control" placeholder="计划名称" datatype="*" nullmsg="请填写计划名称" errormsg="请填写计划名称">
										</div>
									</td>
								</tr>
			        		</c:when>
					...
					<div class="clearfix margin_b10 margin_t20">
						<h3 class="cnt-box-title f16 text-bold">社会实践时间安排</h3>
					</div>

					<table class="table-gray-th">
						<tbody>
							<tr>
								<th width="15%"><small class="text-red">*</small>社会实践申请时间</th>
								<td width="35%">
								...
								<th width="15%"><small class="text-red">*</small>初稿提交截止时间</th>
								<td width="35%">
									<div class="position-relative">
										<input name="submitPracticeEndDt" value="<fmt:formatDate value='${entity.submitPracticeEndDt}' type='date' pattern='yyyy-MM-dd'/>" 
											type="text" class="form-control" placeholder="完成时间" datatype="*" nullmsg="请填写时间" errormsg="时间格式不对" data-role="datetime">
									</div>
								</td>
							</tr>
							<tr>
								<th><small class="text-red">*</small>实践定稿截止时间</th>
								<td>
									<div class="position-relative">
										<input name="confirmPracticeEndDt" value="<fmt:formatDate value='${entity.confirmPracticeEndDt}' type='date' pattern='yyyy-MM-dd'/>" 
											type="text" class="form-control" placeholder="完成时间" datatype="*" nullmsg="请填写时间" errormsg="时间格式不对" data-role="datetime">
									</div>
								</td>
								
								<th><small class="text-red">*</small>实践写作评分时间</th>
								<td>
									<div class="position-relative">
			...
			<c:if test="${action == 'view'}">
				...
				<div class="approval-list clearfix">
					<c:forEach items="${entity.gjtPracticePlanApprovals}" var="approval" varStatus="status">
					...
					<c:if test="${entity.status == 1 && param.isApproval}">
						<dl class="approval-item white-border">
		                  ...
		                  <dd>
		                    <form id="approvalForm" class="theform" action="${ctx}/practicePlan/approval" method="post">
				        	  <input type="hidden" id="planId" name="id" value="${entity.practicePlanId}">
...
;(function(){
	...
	var postForm=$theform.Validform({
	  ...
	  callback:function(data){
    	...
    	if (data.successful) {
	    	window.location.href = ctx + "/practicePlan/list";
	    } else {
