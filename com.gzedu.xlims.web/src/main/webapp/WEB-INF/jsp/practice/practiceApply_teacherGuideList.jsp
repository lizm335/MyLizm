		...
		<div class="box-header with-border">
		  ...
		  <div class="pull-right no-margin">
		  	<c:if test="${isBtnExport}">
		    	<a href="javascript:void(0)" class="btn btn-default btn-sm btn-export"><i class="fa fa-fw fa-plus"></i> 导出指导记录统计表</a>
		    </c:if>
		  </div>
		</div>
		<div class="box-body">
		            		...
		            		<c:when test="${not empty pageInfo.content}">
								<c:forEach items="${pageInfo.content}" var="entity">
									<c:if test="${not empty entity}">
						            	<tr>
						            		<td>
						            			${entity.practicePlanName}<br>
						            			<div class="gray9">（${entity.practicePlanCode}）</div>
						            		</td>
						            		...
						            		<td>
						            			<c:if test="${isBtnView}">
						            				<a href="${ctx}/practiceApply/viewTeacherGuide?practicePlanId=${entity.practicePlanId}&gradeSpecialtyId=${entity.gradeSpecialtyId}&teacherId=${entity.teacherId}" class="operion-item" data-toggle="tooltip" title="查看详情"><i class="fa fa-fw fa-view-more"></i></a>
						            			</c:if>
						            		</td>
						            	</tr>
		            				</c:if>
...
$(function() {
	$(".btn-export").click(function(event) {
		window.open(ctx + "/practiceApply/exportTeacherGuide?" + $("#listForm").serialize());
	});
});
