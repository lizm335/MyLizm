				...
				<div class="box box-primary">
					...
					<form id="inputForm" class="form-horizontal" role="form"
						...
						<div class="box-body">
								...
								<div class="col-sm-3">
									<select name="priRoleInfo.roleId" class="selectpicker show-tick form-control" data-size="6" data-live-search="true">
										...
										<c:forEach items="${orgMagagerRoleMap}" var="role">
											<option value="${role.key}"  <c:if test="${role.key==param['item.priRoleInfo.id']}">selected='selected'</c:if>>${role.value}</option>
										</c:forEach>
									</select>
								</div>
							</div>
							...
							<div class="form-group">
								<label class="col-sm-2 control-label"><small class="text-red">*</small>再次确认密码:</label>
								<div class="col-sm-3">
