    ...
    <div class="box-body pos-rel overlay-wrapper">
        <c:choose>
            <c:when test="${hasPermission}">
                <div class="alert alert-dismissible" style="border-color: #008d4c; background-color: #00a65a !important; color: #fff !important;">
                    <h4><i class="icon fa fa-check"></i>已通过身份验证</h4>
                    数据自动导出中，请稍后......
                </div>
                <script type="text/javascript">
                    $(function () {
                        setTimeout(function () {
                            $('[data-role="export"]').trigger('click')
                        }, 200);
                    })(jQuery);
                </script>
            </c:when>
...
<div class="text-right pop-btn-box pad">
    ...
    <button type="button" class="btn btn-success min-width-90px ${hasPermission?'':'disabled'}" data-role="export" ${hasPermission?'':'disabled'}>导出</button>
</div>
<script type="text/javascript">
    ...
    $('[data-role="get-ver-code"]').click(function(event){
        var hasPhone = $(':input[name="mobileNumber"]').val();
        if(hasPhone==''){
        ...
        $.ajax({
            ...
            data:{},
            async:false,
            url: ctx + "/excelExport/getMessageCode",
    ...
    $(".tel-vercode").keyup(function(event) {
        ...
        if($.trim(this.value)==""){
            ...
            $.ajax({
                ...
                url: ctx + "/excelExport/getCheckCode",
                success:function (data) {
