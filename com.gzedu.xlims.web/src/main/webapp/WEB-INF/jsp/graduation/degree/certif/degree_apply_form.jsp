				...
				<div class="row">
					...
					<div class="col-sm-8">
						<table class="table no-border vertical-middle no-margin text-center margin_t20" height="120" style="table-layout: fixed;">
							<tbody>
								<tr>
									<td width="19%" style="border-left: 1px solid #e5e5e5">${info.degreeCondition==1?'<div class="f20 text-green">已满足</div>':'<div class="f20 text-orange">未满足</div>'}
									...
									<td width="17%" style="border-left: 1px solid #e5e5e5"><c:choose>
											<c:when test="${info.auditState==1}">
												<div class="f20 text-green">省校审核通过</div>
											</c:when>
											<c:when test="${info.auditState==2}">
												<div class="f20 text-red">省校审核不通过</div>
											</c:when>
											<c:when test="${info.auditState==11}">
												<div class="f20 text-red">中央审核不通过</div>
											</c:when>
											<c:when test="${info.auditState==12}">
												<div class="f20 text-red">中央审核不通过</div>
											</c:when>
											<c:otherwise>
							...
							<div style="margin: -1px;">
										...
										<div class="row">
											<div class="col-sm-6 margin_b15">
												<div class="cert-wrap">
													...
													<div class="cert-box has-upload cert-box-1">
														<c:choose>
															<c:when test="${not empty info.idcardFrontUrl}">
															...
															<c:otherwise>
																<a href="javascript:void(0);" class="info-img-box">
																	<img class="info-img" src="http://css.gzedu.com/ouchgzee_com/person_center/v3.0.1/images/act-default.jpg" alt="User Image">
																</a>
															</c:otherwise>
														</c:choose>
											...
											<div class="col-sm-6 margin_b15">
												<div class="cert-wrap">
													...
													<div class="cert-box has-upload cert-box-1">
														<c:choose>
															<c:when test="${not empty info.idcardBackUrl}">
																<a href="javascript:void(0);" class="info-img-box">
										...
										<div class="row">
											<div class="col-sm-6 col-sm-offset-3">
												<div class="cert-wrap">
													<div class="cert-box has-upload cert-box-1">
														<c:choose>
															<c:when test="${not empty info.preliminaryReportUrl}">
																<a href="javascript:void(0);" class="info-img-box">
																	<img class="info-img" src="${info.preliminaryReportUrl}" alt="User Image">
																</a>
																<a href="#" data-role="lightbox" data-large-img="${info.preliminaryReportUrl}" class="light-box">点击放大</a>
															</c:when>
															<c:otherwise>
																<a href="javascript:void(0);" class="info-img-box">
																	<img class="info-img" src="http://css.gzedu.com/ouchgzee_com/person_center/v3.0.1/images/act-default.jpg" alt="User Image">
								...
								<div class="panel panel-default margin-bottom-none flat">
									...
									<div class="panel-body text-center">
										...
										<div class="margin_t30">
											<c:choose>
												<c:when test="${not empty info.thesisUrl}">
													<a href="http://eezxyl.gzedu.com?furl=${info.thesisUrl}" role="button" class="btn min-width-90px btn-warning margin_r10" data-role="addon-preview">在线预览</a>
													<a href="${info.thesisUrl}" role="button" class="btn min-width-90px btn-primary">下载查看</a>
												</c:when>
												<c:otherwise>
													<a href="javascript:void(0);" role="button" class="btn min-width-90px btn-warning disabled margin_r10" >在线预览</a>
													<a href="javascript:void(0);" role="button" class="btn min-width-90px btn-primary disabled">下载查看</a>
												</c:otherwise>
											</c:choose>
										</div>
									</div>
												...
												<table class="table table-bordered vertical-mid text-center" style="table-layout: fixed;">
													<tbody>
														...
														<tr>
															...
															<th>电子注册号</th>
															<td colspan="2">${info.eleRegistrationNumber}</td>
														</tr>
														<tr>
															<th>毕业时间</th>
															<td colspan="4">${info.gjtGraduationPlan.graPlanTitle}</td>
														</tr>
														<tr>
														...
														<tr>
															<td colspan="5">
																<div class="text-left pad">${info.remark }</div>
															</td>
														</tr>
													</tbody>
												</table>
								...
								<%--<c:if test="${info.auditState == 1 }">--%>
									<div class="panel panel-default margin-bottom-none flat">
										...
										<div class="panel-body text-center">
											<textarea class="form-control" id="content" rows="6" placeholder="请审核学生上传的学位资料是否符合要求，并填写审核评语">${info.informationAuditOpinion}</textarea>
										</div>
										<div class="panel-footer text-right">
											<button type="button" class="btn min-width-90px btn-warning margin_r10 btn-save-edit audit" onclick="saveRecord(12);">审核不通过</button>
											<button type="button" class="btn min-width-90px btn-success" data-role="btn-pass" onclick="saveRecord(11);" >审核通过</button>
										</div>
									</div>
								<%--</c:if>--%>
...
function saveRecord(state){
	...
	$.post('${ctx}/graduation/degreeCerif/informationAudit',{
