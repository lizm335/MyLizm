	...
	<section class="content">
		<form id="theform">
			<div class="box margin-bottom-none">
				...
				<div class="box-body">
					<input type="hidden" id="collegeSpecialtyId" value="${info.id}">
					<table class="table-gray-th">
						<tr>
							<th width="15%" class="text-right">院校名称</th>
							<td width="35%">${info.gjtDegreeCollege.collegeName}</td>

							<th width="15%" class="text-right">专业名称</th>
							<td width="35%">
								${info.gjtSpecialty.specialtyName}
							</td>
						</tr>
						<tr>
									...
									<div class="input-group">
										<select id="gradeId" class="form-control margin_r10" datatype="*" nullmsg="请选择入学学期" errormsg="请选择入学学期">
											...
											<c:forEach items="${gradeMap}" var="map">
												<option value="${map.key}" <c:if test="${map.key==info.gradeId}">selected="selected"</c:if>>${map.value}</option>
											</c:forEach>
										</select>
							...
							<td>
								<div class="position-relative">
									<input id="degreeName" class="form-control" type="text" value="${info.degreeName }" placeholder="请填写学位名称" datatype="*" nullmsg="请填写学位名称" errormsg="请填写学位名称">
								</div>
							</td>
						</tr>
					</table>
					...
					<div data-id="conditions-box">
						<c:forEach items="${requirements}" var="item" varStatus="index">
							<div class="box no-border no-shadow margin_b10 conditionsItem">
								<input type="hidden" class="reqId" value="${item.requirementId}" />
								<div class="panel panel-default flat margin-bottom-none">
									<div class="panel-heading clearfix">
										...
										<h3 class="panel-title text-bold f14 pad-t5 reqTitle">条件</h3>
									</div>
									<table class="table">
										<tbody>
											<tr>
													...
													<div class="position-relative">
														<select class="form-control reqType" datatype="*" nullmsg="请选择类型" errormsg="请选择类型">
															...
															<c:forEach items="${baseTypeMap}" var="map">
																<option value="${map.key}"<c:if test="${map.key==item.requirementType}">selected="selected"</c:if> >${map.value}</option>
															</c:forEach>
														</select>
													</div>
												</td>
													...
													<div class="input-group">
														<div class="input-group-btn" style="vertical-align: top;">
															<select class="form-control input-group-select bg-white operator">
																<c:forEach items="${operatorEnum}" var="o">
																	<option value="${o.value}"<c:if test="${o.value==item.operator}">selected="selected"</c:if>>${o.name}</option>
																</c:forEach>
															</select>
														</div>
														<div class="position-relative">
															<input type="text" class="form-control reqParam" value="${item.requirementParam}" placeholder="请填写数值" datatype="/^(0|([1-9]\d*))(\.\d+)?$/" nullmsg="请选择参数" errormsg="请填写数值">
														</div>
											...
											<tr>
												...
												<td>
													<div class="position-relative">
														<textarea class="form-control reqDesc" rows="6" placeholder="请填写描述" datatype="*" nullmsg="请填写描述" errormsg="请填写描述">${item.requirementDesc}</textarea>
													</div>
												</td>
											</tr>
										</tbody>
	...
	(function() {
	    ...
	    var postForm = $theform.Validform({
			...
			beforeSubmit : function(curform) {
			    ...
			    $.post('${ctx}/graduation/degreeCollege/updateDegreeReq', {
					collegeSpecialtyId : $('#collegeSpecialtyId').val(),
					gradeId : $('#gradeId').val(),
	    ...
	    if('${action}' == 'view'){
			...
			$('[data-role="add-conditions"],[data-role="sure"],[data-widget="remove"]').remove();
		}
	})();
    </script>
</body>
