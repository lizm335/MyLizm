				...
				<div class="row">
					...
					<div class="col-sm-8">
						<table class="table no-border vertical-middle no-margin text-center margin_t20" height="120" style="table-layout: fixed;">
							<tbody>
							<tr>
								<td width="15%" style="border-left:1px solid #e5e5e5">
								...
								<td width="25%" style="border-left:1px solid #e5e5e5">
									<c:choose>
										<c:when test="${info.auditState==0}">未审核</c:when>
										<c:when test="${info.auditState==1}"><span class="f20 text-green">省校审核通过</span></c:when>
										<c:when test="${info.auditState==2}"><span class="f20 text-red">省校审核不通过</span></c:when>
										<c:when test="${info.auditState==6}"><span class="f20 text-orange">中央待审核</span></c:when>
										<c:when test="${info.auditState==11}"><span class="f20 text-green">中央审核通过</span></c:when>
										<c:when test="${info.auditState==12}"><span class="f20 text-red">中央审核不通过</span></c:when>
									</c:choose>
									<div class="gray6 margin_t5">审核状态</div>
								</td>
								<td width="15%" style="border-left:1px solid #e5e5e5">
									<c:choose>
										<c:when test="${info.graduationState==0}"><span class="f20 text-red">未毕业</span></c:when>
										<c:when test="${info.graduationState==1}"><span class="f20 text-orange">延迟毕业</span></c:when>
										<c:otherwise><span class="f20 text-green">已毕业</span></c:otherwise>
									</c:choose>
