	...
	$(function() {
		...
		$('#inputForm').bootstrapValidator({
				fields: {
					...
					code: {
						validators: {
							notEmpty: {message:'分校编码不能为空'},
							callback: {
								message: '分校编码已经存在',
								callback: function(value, validator) {
									var validator=true;
									if(oldCode != $("#code").val() && $("#code").val()!==" "){
										$.ajax({
											type : "post",
											url : "${ctx}/organization/orgChild/checkLogin.html",
											dataType:'json',
											data : {code:$('#code').val()},
											async : false,
...
<section class="content-header">
	<button onclick="history.back()" id="btn-back2"  class="btn btn-default btn-sm pull-right min-width-60px" data-role="back-off">返回</button>
	<ol class="breadcrumb">
...
<section class="content">
	...
	<form id="inputForm" class="form-horizontal" role="form" action="${ctx }/organization/orgChild/${action}" method="post">
                <input id="action" type="hidden" name="action" value="${action}">
                <input type="hidden" name="id" value="${entity.id}">
		<div class="box-body">
			<div class="form-horizontal reset-form-horizontal">
				<div class="form-group">
					<label class="col-sm-2 control-label">上级院校</label>
					<div class="col-sm-8">
						<c:if test="${empty orgList}">
							<p class="form-control-static">${entity.parentGjtOrg.orgName}</p>
						</c:if>
						<c:if test="${not empty orgList}">
							<select name="parentGjtOrg.id" class="selectpicker show-tick form-control" data-size="7" data-live-search="true">
								<c:forEach items="${orgList}" var="org">
									<option value="${org.key}"  <c:if test="${org.key==entity.parentGjtOrg.id}">selected='selected'</c:if>>${org.value}</option>
								</c:forEach>
							</select>
						</c:if>
					</div>
				...
				<div class="form-group">
					<label class="col-sm-2 control-label"><small class="text-red">*</small>分校代码</label>
					<div class="col-sm-8">
						<input class="form-control" type="text" id="code" name="code" value="${entity.code}" placeholder="请输入分校代码">
					</div>
				</div>
				<div class="form-group">
					<label class="col-sm-2 control-label"><small class="text-red">*</small>分校名称</label>
					<div class="col-sm-8">
						<input class="form-control" type="text" name="orgName" value="${entity.orgName}" placeholder="请输入分校名称">
					</div>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					<label class="col-sm-2 control-label"><small class="text-red">*</small>院校地址</label>
					<div class="col-sm-8">
						<div class="row">
							<div class="col-sm-4">
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<input class="form-control" type="text" name="gjtSchoolInfo.xxdz" value="${entity.gjtSchoolInfo.xxdz}" placeholder="请输入院校详细地址">
					</div>
				</div>
				<c:if test="${action=='view'}">
				<div class="form-group">
					...
					<div class="col-sm-8">
						...
						&nbsp;&nbsp;&nbsp;&nbsp;
						<input type="radio"  name="schoolModel" value="3" class="flat-red" <c:if test="${entity.schoolModel==3}">checked="checked"</c:if>> 院校模式
					</div>
				</div>
				</c:if>
				<div class="form-group">
...
<!--
				<div class="form-group">
					<label class="col-sm-2 control-label"><small class="text-red">*</small>APP_ID</label>
					<div class="col-sm-8">
						<input class="form-control" type="text" name="gjtSchoolInfo.appid" value="${entity.gjtSchoolInfo.appid}" placeholder="请输入APP_ID">
					</div>
				</div>
-->
				<div class="row">
