            ...
            <div class="box-body">
              <div class="filter-tabs filter-tabs-gutter clearfix">
                <ul class="list-unstyled">
                  ...
                  <li <c:if test="${param.search_EQ_transactionStatus ==3 && param.search_EQ_auditOperatorRole == 4}">class="actived"</c:if> value='4' onclick="choiceStudyQX('4')">学习中心，劝学中（${studyCenterTruancy}）</li>
                  <li <c:if test="${param.search_EQ_transactionStatus ==3 && param.search_EQ_auditOperatorRole == 2}">class="actived"</c:if> value='2' onclick="choiceStudyQX('2')">学生事务部，劝学中（${studentTruancy}）</li>
                  <li <c:if test="${param.search_EQ_transactionStatus ==3 && param.search_EQ_auditOperatorRole == 5}">class="actived"</c:if> value='5' onclick="choiceStudyQX('5')">学籍科，劝学中（${rollTruancy}）</li>
                  <li <c:if test="${param.search_EQ_transactionStatus ==6}">class="actived"</c:if> value='6' onclick="choiceStatus('6')">财务服务部，待核算（${forAccount}）</li>
                  <li <c:if test="${param.search_EQ_transactionStatus ==8 && param.search_EQ_auditOperatorRole == 1}">class="actived"</c:if> value='1' onclick="choiceOnHand('1')">学员，待确认（${unconfirmed}）</li>
                  <li <c:if test="${param.search_EQ_transactionStatus ==8 && param.search_EQ_auditOperatorRole == 0}">class="actived"</c:if> value='0' onclick="choiceOnHand('0')">院长，待处理（${onHand}）</li>
                  <li <c:if test="${param.search_EQ_transactionStatus ==10}">class="actived"</c:if> value='10' onclick="choiceStatus('10')">财务，待登记（${registered}）</li>
                  <li <c:if test="${param.search_EQ_transactionStatus ==8 && param.search_EQ_auditOperatorRole == 5}">class="actived"</c:if> value='5' onclick="choiceOnHand('5')">学籍科，待处理（${rollOnHand}）</li>
                  <li <c:if test="${param.search_EQ_transactionStatus ==12}">class="actived"</c:if> value='12' onclick="choiceStatus('12')">已完成（${finish}）</li>
                  <li <c:if test="${param.search_EQ_transactionStatus ==13}">class="actived"</c:if> value='13' onclick="choiceStatus('13')">撤销退学（${revocation}）</li>
                </ul>
              </div>
                  ...
                  <tbody>
                   <c:forEach items="${pageInfo.content }" var="transaction">
                    <tr>
                      ...
                      <td>
                      	<c:choose>
                         <c:when test="${transaction.transactionStatus==3 && transaction.auditOperatorRole == 4}"><div class="text-green">劝学中</div><small class="gray9">（学习中心）</small></c:when>
                         <c:when test="${transaction.transactionStatus==3 && transaction.auditOperatorRole == 2}"><div class="text-green">劝学中</div><small class="gray9">（学生事务部）</small></c:when>
                         <c:when test="${transaction.transactionStatus==3 && transaction.auditOperatorRole == 5}"><div class="text-green">劝学中</div><small class="gray9">（学籍科）</small></c:when>
                         <c:when test="${transaction.transactionStatus==6 && transaction.auditOperatorRole == 6}"><div class="text-green">待核算</div><small class="gray9">（财务服务部）</small></c:when>
                         <c:when test="${transaction.transactionStatus==8 && transaction.auditOperatorRole == 1}"><div class="text-green">待确认</div><small class="gray9">（学员）</small></c:when>
                         <c:when test="${transaction.transactionStatus==8 && transaction.auditOperatorRole == 0}"><div class="text-green">待处理</div><small class="gray9">（院长）</small></c:when>
                         <c:when test="${transaction.transactionStatus==10 && transaction.auditOperatorRole == 7}"><div class="text-green">待登记</div><small class="gray9">（财务）</small></c:when>
                         <c:when test="${transaction.transactionStatus==8 && transaction.auditOperatorRole == 5}"><div class="text-green">待处理</div><small class="gray9">（学籍科）</small></c:when>
...
//劝学状态
function choiceStudyQX(flag){
	$("#auditOperator").val('');
	$("#transactionStatus").val('');
	$("#auditOperator").val(flag);
	$("#transactionStatus").val('3');
	$("#listForm").submit();
}
//待处理状态
function choiceOnHand(flag){
	$("#auditOperator").val('');
	$("#transactionStatus").val('');
	$("#auditOperator").val(flag);
	$("#transactionStatus").val('8');
	$("#listForm").submit();
}
</script>
</body>
