...
<body class="inner-page-body">
	<section class="content-header">
		...
		<ol class="breadcrumb oh">
			...
			<li><a href="#">学籍管理</a></li>
			<li><a href="${ctx}/edumanage/roll/list">学籍信息</a></li>
			<li class="active">查看学籍详情</li>
		</ol>
	</section>
		...
		<div class="box">
							...
							<div class="col-xs-6 col-sm-4 pad-b5">
								<b>邮箱:</b>
								 ...
								 <shiro:hasPermission name="/personal/index$privacyJurisdiction">
									...
									<shiro:lacksPermission name="/personal/index$privacyJurisdiction">
										<c:if test="${not empty item.dzxx }">
										${fn:substring(item.dzxx, 0, 3)}******${fn:substring(item.dzxx, 6, (item.dzxx).length())}
										</c:if>
									</shiro:lacksPermission>
			...
			<div class="box-footer">
				<div class="row stu-info-status">
					<div class="col-xs-4">
						<c:choose>
							<c:when test="${item.xjzt=='2'||item.xjzt=='8'}">
								<div class="text-green f24 text-center"><dic:getLabel typeCode="StudentNumberStatus" code="${item.xjzt }" /></div>
							</c:when>
							<c:otherwise>
								<div class="text-red f24 text-center"><dic:getLabel typeCode="StudentNumberStatus" code="${item.xjzt }" /></div>
							</c:otherwise>
						</c:choose>
					...
					<div class="col-xs-4">
						<div class="text-green f24 text-center"><dic:getLabel typeCode="USER_TYPE" code="${item.userType}" /></div>
													...
													<td>
														...
														<shiro:hasPermission name="/personal/index$privacyJurisdiction">
															...
															<shiro:lacksPermission name="/personal/index$privacyJurisdiction">
																...
																${fn:substring(item.sfzh,0, 4)}******${fn:substring(item.sfzh,14, (item.sfzh).length())}
																</c:if>
															</shiro:lacksPermission>
												...
												<table class="table-gray-th">
													<tr>
														...
														<td>
															...
															<shiro:lacksPermission name="/personal/index$privacyJurisdiction">
																<c:if test="${not empty item.lxdh }">
																${fn:substring(item.lxdh,0, 3)}******${fn:substring(item.lxdh,8, (item.lxdh).length())}
																</c:if>
															</shiro:lacksPermission>
														...
														<c:when test="${isUndergraduateCourse }">
															...
															<tr>
																...
																<td required>
																	...
																	<shiro:lacksPermission name="/personal/index$privacyJurisdiction">
																		<c:if test="${not empty item.exedunum }">
																		${fn:substring(item.exedunum,0, 4)}******${fn:substring(item.exedunum,14, (item.exedunum).length())}
																		</c:if>
																	</shiro:lacksPermission>
