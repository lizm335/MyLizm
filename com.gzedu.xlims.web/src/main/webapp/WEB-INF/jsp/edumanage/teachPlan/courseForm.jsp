...
<body>
<form id="inputForm" class="form-horizontal" role="form"	action="${ctx }/edumanage/teachPlan/${action}" method="post">
<input type="hidden" name="gradeId" value="${entity.gradeId}" id="gradeId"/>
<input type="hidden" name="kkzy" value="${entity.kkzy}" id="kkzy"/>
<input type="hidden" name="kkxq" value="${entity.kkxq}" id="kkxq"/>
<input type="hidden" name="teachPlanId" value="${entity.teachPlanId}"/>
		...
		<div class="slim-Scroll">
			...
			<div class="table-responsive margin-bottom-none">
				<table class="table-gray-th">
					<tr>
						<th width="15%" class="text-right">学期：</th>
						<td width="35%">第${entity.kkxq}学期</td>
						<th width="15%" class="text-right"><small class="text-red">*</small>课程名称：</th>
						<td width="35%">
							<c:if test="${not empty entity.gjtCourse }">
								${entity.gjtCourse.kcmc}<span class="gray9"> （${entity.gjtCourse.kch }） </span>
							</c:if>
						</td>
					</tr>
					<tr>
						...
						<td width="35%">
							${courseModeMap[entity.kclbm]}
						</td>
						<th class="text-right">
						课程属性：</th>
						<td>
							<select class="form-control" name="kcsx">
			                  <option value="0" <c:if test="${entity.kcsx=='0'}"> selected="selected"</c:if>>必修</option>
			                  <option value="1" <c:if test="${entity.kcsx=='1'}"> selected="selected"</c:if>>选修</option>
			                  <option value="2" <c:if test="${entity.kcsx=='2'}"> selected="selected"</c:if>>补修</option>
			                </select>
						</td>
						
					</tr>
					<tr>
						<th width="15%" class="text-right">课程类型：</th>
						<td width="35%">
							<select class="form-control" name="kclx">
								<option value="">请选择</option>
			                  	<option value="0" <c:if test="${entity.kclx=='0'}"> selected="selected"}</c:if>>统设</option>
			                  	<option value="1" <c:if test="${entity.kclx=='1'}"> selected="selected"}</c:if>>非统设</option>
			                </select>
						</td>
						<th class="text-right">课程类别：</th>
						<td>
							<select  class="form-control" name="kclb">
							  <option value="">请选择</option>
			                  <option value="0" <c:if test="${entity.kclb=='0'}"> selected="selected"</c:if>>普通课程</option>
			                  <option value="1" <c:if test="${entity.kclb=='1'}"> selected="selected"</c:if>>毕业设计课程</option>
			                  <option value="2" <c:if test="${entity.kclb=='2'}"> selected="selected"</c:if>>社会实践课程</option>
			                  <option value="3" <c:if test="${entity.kclb=='3'}"> selected="selected"</c:if>>学位课程</option>
			                </select>
						</td>
						
					</tr>
					<tr>
						<th class="text-right"> 替换课程：</th>
						<td>
							<div class="pull-right margin_l10">
								<a role="button" class="btn btn-default" <c:if test="${action!='viewPlan' }">href="${ctx}/edumanage/specialty/choiceCourseList?action=replaceCourse" data-role="select-pop"</c:if> data-target="s2">选择课程</a>
							</div>
							<div class="select2-container select2-container--default show oh">
								<div class="select2-selection--multiple">
									<ul class="select2-selection__rendered select-container-ul" data-id="s2">
										<c:if test="${not empty entity.gjtReplaceCourse }">
						...
						<td>
							<div class="pull-right margin_l10">
								<a href="${ctx}/edumanage/specialty/choiceTextbookList/1?courseId=${param.courseId}" role="button" class="btn btn-default" data-role="select-pop" data-target="s3">选择教材</a>
							</div>
							<div class="select2-container select2-container--default show oh">
								<div class="select2-selection--multiple">
					...
					<tr>
						...
						<td>
							<div class="pull-right margin_l10">
								<a href="${ctx}/edumanage/specialty/choiceTextbookList/2?courseId=${param.courseId}" role="button" class="btn btn-default" data-role="select-pop" data-target="s4">选择教材</a>
							</div>
							<div class="select2-container select2-container--default show oh">
								<div class="select2-selection--multiple">
			...
			<div class="table-responsive margin-bottom-none">
				<table class="table-gray-th">
					<tr>
						...
						<td width="35%">
							<select  data-role="choiceExamList" class="form-control" name="ksfs" id="ksfs">
			                  <c:forEach items="${ksfsMap}" var="map">
										<option value="${map.key}"  <c:if test="${map.key==entity.ksfs}">selected='selected'</c:if>>${map.value}</option>
									</c:forEach>
			                </select>
						</td>
					...
					<tr>
						<th width="15%" class="text-right">考试形式：</th>
						<td width="35%">
							<select class="form-control" name="examStyle">
								<option value="">请选择</option>
						...
						<th width="15%" class="text-right">考试单位：</th>
						<td width="35%">
							<select class="form-control" name="ksdw">
								<option value="">请选择</option>
								<option value="1" <c:if test="${entity.ksdw==1}">selected='selected'</c:if>>省</option>
								<option value="2" <c:if test="${entity.ksdw==2}">selected='selected'</c:if>>中央</option>
							</select>
						</td>
					</tr>
				</table>
			...
			<div class="table-responsive margin-bottom-none">
				<table class="table-gray-th">
					<tr>
						...
						<td width="35%">
							<input class="form-control" type="number" placeholder="学时" name="xs" value="${entity.xs}">
						</td>

						<th width="15%" class="text-right"><small class="text-red">*</small>学分：</th>
						<td width="35%">
							<input class="form-control" type="number" placeholder="学分" name="xf" value="${entity.xf}">
						</td>
					</tr>
					<tr>
						<th class="text-right">形成性考核比例：</th>
						<td>
							<input id="studyRatio" class="form-control" type="number" placeholder="1-100之间的值" name="kcxxbz" value="${entity.kcxxbz}">
						</td>

						<th class="text-right">终结性考核比例：</th>
						<td>
							<input id="examRatio" class="form-control" type="number" placeholder="1-100之间的值" name="kcksbz" value="${entity.kcksbz}">
						</td>
					</tr>
					<tr>
						<th class="text-right">考核说明：</th>
						<td colspan="3">
							<textarea class="form-control" rows="5" placeholder="考核说明" name="examRemark">${entity.examRemark }</textarea>
						</td>
					</tr>
				</table>
			</div>
...
$('[data-role="sure"]').click(function(event) {	
	 var url = "${ctx}/edumanage/teachPlan/${action}";
	 $.ajax({
		    type: 'post',
		    url:url,
...
//关闭 弹窗
 ...
 $("[data-role='choiceExamList']").click(function(event) {		
		var url = "${ctx}/edumanage/specialty/choiceExamList?examType="+$('#ksfs').val();
