		    	 ...
		    	 <div class="col-xs-4">
		            <div class="form-group">
		              ...
		              <div class="col-xs-9">
		                ...
		                <input type="hidden" name="notInCoursIds" value="${param.notInCoursIds}" />
		              </div>
		            </div>
		          </div>
		          <div class="col-xs-4">
						...
						<div class="col-xs-9">
							<select name="search_EQ_syzy" class="selectpicker show-tick form-control" 	data-size="5" data-live-search="true">
								...
								<c:forEach items="${syzyMap}" var="map">
									<option value="${map.key}" <c:if test="${map.key==param.search_EQ_syzy}">selected='selected'</c:if>>${map.value}</option>
								</c:forEach>
							</select>
						</div>
					</div>
...
/* 	$("[data-role='single-page']").click(function(event) {
	...
	$("[data-role='save-data']").click(function(event) {
	  ...
	  if(self!==parent){
		    if(parent.choiceReplaceCourseCallback){
		 	var name=$.trim($(".batch-teacher tr.on").find(".name").text());
			    var courseId=$.trim($(".batch-teacher tr.on").find(".courseId").val());
			    var code=$.trim($(".batch-teacher tr.on").find(".code").text());
			    var isEnabled=$.trim($(".batch-teacher tr.on").find(".isEnabled").val());
				parent.choiceReplaceCourseCallback(courseId,name,code,isEnabled);
		    }
		}
	 	//parent.$.closeDialog(frameElement.api);
	});
