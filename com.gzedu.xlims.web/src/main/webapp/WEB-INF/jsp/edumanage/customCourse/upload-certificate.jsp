...
<div class="box no-border no-shadow margin-bottom-none">
	<div class="box-header with-border">
		<h3 class="box-title">上传授课凭证</h3>
		<%-- <a href="${ctx }/excel/download?name=授课凭证导入模板.xls" target="_blank" class="btn">
     		下载标准模板
	 	</a>  --%>
	</div>
	<div class="box-body scroll-box">
		<div class="clearfix margin_b10">
			<form id="excelFileForm" class="form-horizontal" action="" method="post">
				<input type="hidden" id="grantCoursePlanId" name="grantCoursePlanId" value="${plan.grantCoursePlanId }">
				<input type="hidden" id="fileName" name="fileName" value="${plan.fileName }"> 
				<input type="hidden" id="fileUrl" name="fileUrl" value="${plan.fileUrl }">
			</form>
			<!-- 0:未上传,1:上传中, 2:已上传 -->
			<c:if test="${customCourse.scoreStatus ==0 or customCourse.scoreStatus ==3 or customCourse.scoreStatus ==6}">
				<!-- <button onclick="uploadCertificate()" 
					 class="btn btn-primary btn-sm pull-right margin_l10">上传文件</button> -->
				
				<a href="/edumanage/custom/course/upload?type=certificate&id=${plan.grantCoursePlanId }" data-role="import" 
					 class="btn btn-primary btn-sm pull-right margin_l10">上传文件</a>
			</c:if>
		...
		<div class="clearfix margin_t10">
			<!-- 0:未上传,1:上传中, 2:已上传 -->
			<c:if test="${customCourse.scoreStatus ==0 or customCourse.scoreStatus ==3 or customCourse.scoreStatus ==6}">
				<button class="btn btn-primary btn-sm pull-right margin_l10 btn-add-img-addon" type="button">添加扫描件</button>
			</c:if>
			<div class="margin_t5 oh">
...
<div class="text-right pop-btn-box pad">
	...
	<button type="button" class="btn btn-success min-width-90px margin_l15" data-role="export">确定</button>
</div>
</c:if>
