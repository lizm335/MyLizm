...
<section class="content">
	<form id="listForm" class="form-horizontal" action="${ctx}/edumanage/onlinelesson/queryOnlineActivityDetail">
			<div class="box">
				<div class="box-header with-border">
					<div class="media pad-t10 pad-b10 question-item">
						<div class="media-left">
							<c:if test="${empty(onlineLesson.imgUrl) }">
								<img src="${css}/common/v2/temp/demo_01.jpg" width="210" height="140">
							</c:if>
							<c:if test="${not empty(onlineLesson.imgUrl) }">
								<img src="${onlineLesson.imgUrl}" width="210" height="140">
							</c:if>
						</div>
						<div class="media-body">
							<h4 class="media-heading">${onlineLesson.onlinetutorName}</h4>
							<div class="gray9">
								<div class="margin_t5">直播时间：
									 <fmt:formatDate value="${onlineLesson.onlinetutorStart}" type="date" pattern="yyyy-MM-dd HH:mm"/>~<fmt:formatDate value="${onlineLesson.onlinetutorFinish}" type="date" pattern="HH:mm"/>
								</div>
								<div class="margin_t10 txt gray9" data-id="broadcast-summary">${onlineLesson.onlinetutorDesc}</div>
								<ul class="list-unstyled margin_t10">
									<li>
										<span class="pad-r20">  
											层次：${empty(onlineLesson.getRangeIds(8))?'全部':'' }
											<c:forEach items="${onlineLesson.getRangeIds('8') }" var="item">
												${pyccMap[item]}
											</c:forEach>
										</span> 
										<span>
  											年级：${empty(onlineLesson.getRangeIds(4))?'全部':'' }
											<c:forEach items="${onlineLesson.getRangeIds(4) }" var="item">
												${gradeMap[item]}
											</c:forEach>
										</span>
									</li>
									<li>
										专业：${empty(onlineLesson.getRangeIds(5))?'全部':'' }
										<c:forEach items="${onlineLesson.getRangeIds(5) }" var="item">
											${gradeMap[item]}
										</c:forEach>
									</li>
									<li>
				...
				<div class="box-body pad-t15 pad-b15">
					<div class="clearfix">
						<div class="pull-right margin_t10 text-nowrap">
							<c:choose>
								<c:when test="${onlineLesson.status == 1 || onlineLesson.status == 0}">
									<span class="inline-block vertical-middle margin_r10">${onlineLesson.status==0?'距离直播开始还有':'直播已开始' } ${diff}</span>
									<a type="button" target="_blank" href="${onlineLesson.teacherjoinurl}?nickname=${userName}&token=${onlineLesson.teachertoken}" class="btn btn-warning btn-md" style="width: 120px;">进入直播</a>
									<div class="text-right margin_t5">
										<a target="_blank" href="${onlineLesson.teacherjoinurl}?nickname=${userName}&token=111111" class="text-blue inline-block text-center" style="width: 120px;">
											<u>助教入口</u>
										</a>
									</div>
								</c:when>
								<c:when test="${onlineLesson.status eq 2}">
									<c:if test="${not empty(onlineLesson.videojoinurl) }">
										<a target="_blank" href="${onlineLesson.videojoinurl}?nickname=${userName}&token=${onlineLesson.videotoken}" class="btn btn-warning btn-md" style="width: 120px;">查看录播</a>
									</c:if>
						...
						<div class="oh">
							<div class="row stu-info-status text-center">
								<div class="col-sm-2 col-xs-6" style="border-left: 1px solid #eee;">
									<div class="f20">${onlineLesson.allNum}</div>
									<div class="gray9">需辅导人数</div>
								</div>
								<div class="col-sm-2 col-xs-6">
									<div class="f20">${onlineLesson.status==0?'--':onlineLesson.joinNum}</div>
								...
								<div class="col-sm-2 col-xs-6">
									<c:choose>
	                                    <c:when test="${onlineLesson.status eq 1}"><div class="f20 text-orange">直播中</div></c:when>
	                                    <c:when test="${onlineLesson.status eq 0}"><div class="f20 text-orange">待开始</div></c:when>
    ...
    <div class="box">
        ...
        <div class="box-body">
            ...
            <ul class="list-unstyled list-export">
                <li data-type="">导出全部（${allNum}）</li>
                <li data-type="1">导出已参与（${joinNum}）</li>
                <li data-type="0">导出未参与（${unjoinNum}）</li>
                <li data-type="2">导出已看录播（${viewNum}）</li>
            </ul> --%>
...
<script type="text/javascript">
    $(function() {
        ...
        $("#btn_resee").click(function (event) {
            ...
            console.log(teacherjoinurl,teachertoken);
