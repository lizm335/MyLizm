							...
							<c:forEach items="${pageInfo.content}" var="entity">
								<c:if test="${not empty entity}">
									<tr>
										...
										<td><span class="code">${entity.kch}</span></td>
										<td class="order-num">
											<input type="hidden" class="courseId" value="${entity.courseId}"/>
											<input type="hidden" class="orderNum" value="${entity.kch}"/>
											<div class="name">${entity.kcmc}</div>
										</td>
...
/* 	$("[data-role='single-page']").click(function(event) {
	...
	$("[data-role='save-data']").click(function(event) {
	  ...
	  if(self!==parent){
	      	if($(".batch-teacher tr.on").length==0){
	      	    alert('请先选择课程');
	      	    return;
	      	}
	      	...
	      	$(".batch-teacher tr.on").each(function(index, el) {
			    ...
			    var order=$.trim($(this).find(".orderNum").val());
			    orders.push(order);
			});
	      	
		    if(parent.choiceCourseCallback){
				parent.choiceCourseCallback(courseIds, names, orders);
		    }
		}
	 	parent.$.closeDialog(frameElement.api);
	});
