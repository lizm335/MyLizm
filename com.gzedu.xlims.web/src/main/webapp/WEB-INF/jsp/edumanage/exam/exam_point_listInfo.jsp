...
<section class="content">
	<form id="listForm" class="form-horizontal" action="${ctx}/exam/new/point/list" method="post">
	<input type="hidden" id="EXAM_TYPE" name="EXAM_TYPE" value="${EXAM_TYPE}">
	<div class="nav-tabs-custom no-margin">
	 <ul class="nav nav-tabs nav-tabs-lg">
		<li <c:if test="${EXAM_TYPE == 8 || empty EXAM_TYPE}">class="active"</c:if>><a href="list?EXAM_TYPE=8" target="_self">笔试</a></li>
		<li <c:if test="${EXAM_TYPE == 11}">class="active"</c:if>><a href="list?EXAM_TYPE=11" target="_self">机考</a></li>
		 <li><a href="baseList" target="_self">考点列表</a></li>
	 </ul>
			...
			<div class="fr">
				<div class="btn-wrap fl">
					<shiro:hasPermission name="/exam/new/point/list$create">
						<a href="javascript:void(0);" class="btn btn-default btn-sm" data-role="select-pop"> <i class="fa fa-fw fa-plus"></i> 批量新增考点</a>
					</shiro:hasPermission>
				</div>
			</div>
		</div>
				...
				<table class="table table-bordered table-striped vertical-mid text-center table-font">
					<thead>
		              <tr>
						...
						<th>详细地址</th>
						<th>适用范围</th>
		                <c:if test="${EXAM_TYPE=='8'}">
		                	<th>笔考已预约人数</th>
		                </c:if>
		                <c:if test="${EXAM_TYPE=='11'}">
		                	<th>机考已预约人数</th>
		                </c:if>
		                <th>已设考场</th>
		                <th>考场座位数</th>
		            		...
		            		<c:when test="${not empty pageInfo.content}">
		            			<c:forEach items="${pageInfo.content }" var="entity">
		            				<tr>
				            			...
				            			<c:if test="${entity.PEN_TEST_COUNT - entity.SEATS_COUNT > 0 && entity.EXAM_TYPE=='8'}">
				            				<td class="text-red">座位不足</td> 
				            			</c:if>
				            			<c:if test="${entity.PEN_TEST_COUNT - entity.SEATS_COUNT <= 0 && entity.EXAM_TYPE=='8'}">
				            				<td class="text-green">座位充足</td> 
				            			</c:if>
				            			<c:if test="${entity.COMPUTER_TEST_COUNT - entity.SEATS_COUNT > 0 && entity.EXAM_TYPE=='11'}">
				            				<td class="text-red">座位不足</td> 
				            			</c:if>
				            			<c:if test="${entity.COMPUTER_TEST_COUNT - entity.SEATS_COUNT <= 0 && entity.EXAM_TYPE=='11'}">
				            				<td class="text-green">座位充足</td> 
...
$('#exam_batch_id').change(function(){
	...
	if(exam_batch_id!=''){
		$.ajax({
			type:"get",
			url:"${ctx}/exam/new/room/queryExamPoint?EXAM_BATCH_ID="+exam_batch_id+"&EXAM_TYPE="+EXAM_TYPE,
			dataType:"json",
			data:"",
...
$("[data-role='select-pop']").click(function(event) {
	...
	$.mydialog({
	  ...
	  height:700,
	  zIndex:11000,
	  content: 'url:' + ctx + '/exam/new/point/choiceMultiPointList?examBatchId='+$('#exam_batch_id').val()+'&EXAM_TYPE=' + $("#EXAM_TYPE").val()
	});
});

//选择考试后回调
function choicePointCallback(pointIds, names, examBatchId) {
	$.ajax({
		  url: ctx + "/exam/new/point/batchAddPoint",
		  dataType: 'json',
		  data: {examBatchId: examBatchId, pointIds: pointIds},
		  type: 'post',
