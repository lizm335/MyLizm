...
<div class="box no-border">
	<div class="box-header with-border">
		<h3 class="box-title">批量导入排考数据</h3>
	</div>
	<div class="pop-content">
		<ol class="list-unstyled clearfix process-list">
			<li class="process-step1 actived cur"><i></i>1.下载未排考数据</li>
				...
				<div class="box-body process-cnt-box">
					<div class="media left10 pad-t15 margin_r15">
						...
						<div class="media-body">
							请选择你要指定考场排考的考试计划和考试类型
						</div>
					</div>
					<div class="pad-t10 col-xs-8 col-xs-offset-2">
					...
					<div class="text-center margin_t10">
						<a class="btn btn-primary btn-lg download" style='width:220px;' href="javascript:void(0)">下载未排考学员</a>
					</div>
					<div class="text-center text-red f12 margin_t10">
					...
					<div class="table-block full-width" style="height:100%;">
						<div class="table-row">
							<div class="table-cell-block vertical-mid text-center">
								...
								请选择你要导入的《排考数据导入表》，并选择需要导入到的考试计划
								</p>
								<form id="uploadForm" action="${ctx}/exam/new/student/room/importExamStudentRoom" enctype="multipart/form-data" method="post">
									<div class="pad-t30 col-xs-8 col-xs-offset-2">
										<select name="examBatchCode" class="selectpicker show-tick form-control" data-size="5" data-live-search="true" >
											<c:forEach items="${batchMap}" var="map">
												<option value="${map.key}">${map.value}</option>
											</c:forEach>
										</select>
...
$(function(){
	...
	$(".download").click(function(){
    	...
    	window.open(ctx+"/exam/new/student/room/exportNoArrange?examBatchCode="+examBatchCode+"&examType="+examType);
    });
    	...
    	$("#uploadForm").ajaxSubmit({
			...
			success: function(data) {
				if (data.successful) {
					...
					$("#successFileName").attr("href", ctx+"/excelImport/downloadResult?path=examStudentRoom&name="+data.successFileName);
					$("#failedFileName").attr("href", ctx+"/excelImport/downloadResult?path=examStudentRoom&name="+data.failedFileName);
