...
<div class="box no-border">
    <div class="box-header with-border">
        <c:if test="${type eq 'supervisorTeacher'}">
            <h3 class="box-title">批量导入督导教师</h3>
        </c:if>
        <c:if test="${type eq 'coachTeacher'}">
            <h3 class="box-title">批量导入辅导教师</h3>
        </c:if>
        <c:if test="${type eq 'headTeacher'}">
            <h3 class="box-title">批量导入班主任</h3>
        </c:if>
        <c:if test="${type eq 'classTeacher'}">
            <h3 class="box-title">批量导入任课教师</h3>
        </c:if>
    </div>
    <div class="pop-content">
        ...
        <div class="step-box">
						...
						<div class="media pad-t30 left10 margin_t10 margin_r15">
							...
							<c:if test="${type eq 'supervisorTeacher'}">
								<div class="media-body">
									为了方便你的工作，我们已经准备好了《督导教师导入表》的标准模板<br> 你可以点击下面的下载按钮，下载标准模板。
								</div>
							</c:if>
							<c:if test="${type eq 'coachTeacher'}">
								<div class="media-body">
									为了方便你的工作，我们已经准备好了《辅导教师导入表》的标准模板<br> 你可以点击下面的下载按钮，下载标准模板。
								</div>
							</c:if>
							<c:if test="${type eq 'headTeacher'}">
								<div class="media-body">
									为了方便你的工作，我们已经准备好了《班主任表》的标准模板<br> 你可以点击下面的下载按钮，下载标准模板。
								</div>
							</c:if>
							<c:if test="${type eq 'guideTeacher'}">
								<div class="media-body">
									为了方便你的工作，我们已经准备好了《论文指导教师导入表》的标准模板<br> 你可以点击下面的下载按钮，下载标准模板。
								</div>
							</c:if>
							<c:if test="${type eq 'replyTeacher'}">
								<div class="media-body">
									为了方便你的工作，我们已经准备好了《论文答辩教师导入表》的标准模板<br> 你可以点击下面的下载按钮，下载标准模板。
								</div>
							</c:if>
							<c:if test="${type eq 'practiceTeacher'}">
								<div class="media-body">
									为了方便你的工作，我们已经准备好了《社会实践教师入表》的标准模板<br> 你可以点击下面的下载按钮，下载标准模板。
								</div>
							</c:if>
							<c:if test="${type eq 'classTeacher'}">
								<div class="media-body">
									为了方便你的工作，我们已经准备好了《任课教师入表》的标准模板<br> 你可以点击下面的下载按钮，下载标准模板。
								</div>
							</c:if>
						</div>
						<div class="text-center pad-t30 margin_t10">
                        <c:if test="${type ne 'headTeacher'}">
								<a href="${ctx }/excel/download?name=教职工导入模板.xls" target="_blank" class="btn btn-primary btn-lg"  style='width:220px;'>
		                             下载标准模板
		                        </a>                            
                        </c:if>
                        <c:if test="${type eq 'headTeacher'}">
                        	<shiro:hasPermission name="/edumanage/teachEmployee/list$schoolModel">
	                           <a href="${ctx }/excel/download?name=班主任导入模板(教辅云).xls" target="_blank" class="btn btn-primary btn-lg"  style='width:220px;'>
	                             下载标准模板
	                       		 </a>  
                       		 </shiro:hasPermission>  
                       		 <shiro:lacksPermission  name="/edumanage/teachEmployee/list$schoolModel">
                       		 		<a href="${ctx }/excel/download?name=班主任导入模板.xls" target="_blank" class="btn btn-primary btn-lg"  style='width:220px;'>
	                             		下载标准模板
	                       		 	</a>  
            ...
            <div class="step-item" style="display:none">
                <form action="${ctx }/edumanage/teachEmployee/saveImport.html?type=${type}" id="uploadForm" method="post" enctype="multipart/form-data">
        	...
        	$("#uploadForm").ajaxSubmit({
    			...
    			success: function(data) {
    				if (data.successful) {
    					...
    					$("#successFileName").attr("href", ctx+"/excelImport/downloadResult?path=employee&name="+data.successFileName);
    					$("#failedFileName").attr("href", ctx+"/excelImport/downloadResult?path=employee&name="+data.failedFileName);
