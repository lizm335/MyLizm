...
<body class="inner-page-body" style="padding-top: 0px;">
	<section class="content">
		...
		<div class="box no-border margin-bottom-none">
			<form id="inputForm" class="theform" role="form" action="${ctx }/edumanage/schoolRollInfo/update" method="post">
				...
				<ul class="nav nav-tabs nav-tabs-lg">
					...
					<div class="pull-right no-margin">
						<button type="button" class="btn min-width-90px btn-success margin_r10 margin_t10 btn-save" val="1" data-form-id="5">保&nbsp;&nbsp;&nbsp;&nbsp;存</button>
					</div>
					<div class="pull-right no-margin" style="padding: 10px; width: 500px;">
						<input type="text" class="form-control" name="updatedBy" value="" maxlength="30" placeholder="请输入这次修改的描述,如：20171027修改姓名">
					</div>
				</ul>
				<div class="tab-content">
					...
					<div class="tab-pane" id="tab_notice_3">
							...
							<c:forEach var="record" items="${flowRecordList}" varStatus="s">
								...
								<c:if test="${record.auditOperatorRole!=1}">
									<dl class="approval-item" <c:if test="${s.index==(fn:length(flowRecordList)-1)}">style="border-left: 4px solid rgb(255,255,255);"</c:if>>
										...
										<c:if test="${record.auditState==0}">
											<c:if test="${item.xjzt=='5'}"> <!-- 已退费，无需审核 -->
												<dd>
													<div class="txt">
														<p style="color: #dd4b39;">已退费，无需审核</p>
														<div class="gray9 text-right"></div>
														<i class="arrow-top"></i>
													</div>
												</dd>
											</c:if>
						...
						<div class="form-horizontal" style="margin: 20px auto;">
							<div class="form-group">
								<label for="inputAuditState" class="col-sm-1 control-label">审批状态</label>
								<div class="col-sm-5">
									<select name="auditState"  class="selectpicker show-tick form-control" data-size="5">
										<option value="1" <c:if test="${'1'==item.gjtSignup.auditState}">selected='selected'</c:if>>审核通过</option>
										<option value="0" <c:if test="${'0'==item.gjtSignup.auditState}">selected='selected'</c:if>>审核不通过</option>
										<option value="3" <c:if test="${'1'!=item.gjtSignup.auditState&&'0'!=item.gjtSignup.auditState}">selected='selected'</c:if>>待审核</option>
									</select>
								</div>
							</div>
						</div>
