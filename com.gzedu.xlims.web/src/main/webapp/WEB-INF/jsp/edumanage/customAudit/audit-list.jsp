	        ...
	        <div class="row pad-t15">
			  <div class="col-xs-6 col-sm-4">
	            <div class="form-group">
	              ...
	              <div class="col-sm-9">
	                <select name="orgId"  class="selectpicker show-tick form-control"	data-size="6" data-live-search="true">
						<option value="" selected="selected">请选择</option>
						<c:forEach items="${studyCenterMap}" var="map">
							<option value="${map.key}" <c:if test="${map.key==param['orgId']}">selected='selected'</c:if>>${map.value}</option>
						</c:forEach>
					</select>
	              </div>
	            </div>
	            ...
	            <div class="form-group">
	              ...
	              <div class="col-sm-9">
	                <select name="scoreStatus" class="form-control selectpicker show-tick form-control" data-size="5" data-live-search="true">
	                  ...
	                  <option value="6" <c:if test="${param['scoreStatus']==3 }">selected="selected"</c:if> >上传中</option>
	                </select>
	              </div>
	            </div>
	          </div>
				...
				<table class="table table-bordered table-striped vertical-mid text-center table-font">
		            ...
		            <tbody>
		            	...
		            	<tr id="${info.ORG_ID }-${info.TEACH_PLAN_ID }">
		            		...
		            		<td>
		            			所属专业：<span class="specialty">${info.SPECIALTY_NAME }<small class="gray9">(${info.RULE_CODE })</small></span><br>
								学院课程：<span class="course">${info.COURSE_NAME }<small class="gray9">(${info.COURSE_CODE })</small></span>
							</td>
		            		<td class="orgName">${info.ORG_NAME }</td>
		            		...
		            		<td data-orgId="${info.ORG_ID }" data-teachPlanId="${info.TEACH_PLAN_ID }" data-customCourseId="${info.CUSTOM_COURSE_ID }"
		            			...
		            			<c:choose>
		            				<c:when test="${info.PLAN_STATUS == 1 }">
		            					<a href="#" class="operion-item audit-plan" data-toggle="tooltip" data-container="body" title="审核授课计划"><i class="fa fa-shxxjl"></i></a>
		            				</c:when>
		            				<c:when test="${info.PLAN_STATUS != 0 }">
		            					<a href="#" class="operion-item audit-plan" data-toggle="tooltip" data-container="body" title="查看授课计划"><i class="fa fa-view-more"></i></a>
		            				</c:when>
		            			</c:choose>
		            			<c:choose>
		            				<c:when test="${info.CERTIFICATE_STATUS == 1 or info.CERTIFICATE_STATUS == 2}">
		            					<a href="#" class="operion-item audit-certificate" data-toggle="tooltip" data-container="body" title="查看授课凭证"><i class="fa fa-search-plus"></i></a>
		            				</c:when>
		            			</c:choose>
	            				<c:choose>
	            					<c:when test="${info.SCORE_STATUS == 1}">
		            					<a href="#" class="operion-item audit-score" data-toggle="tooltip" data-container="body" title="审核授课凭证和成绩"><i class="fa fa-ksyy"></i></a>
		            				</c:when>
		            				<c:when test="${info.SCORE_STATUS != 0}">
		            					<a href="#" class="operion-item audit-score" data-toggle="tooltip" data-container="body" title="查看授课凭证和成绩"><i class="fa fa-search"></i></a>
		            				</c:when>
	            				</c:choose>
		            		</td>
		            	</tr>
...
$(".box-body").find(".audit-plan").unbind("click").bind("click",function() {
	var custom = buildCustom($(this).parent());
	location.href="toAuditPlan/"+custom.customCourseId;
});
$(".box-body").find(".audit-certificate").unbind("click").bind("click",function() {
	var custom = buildCustom($(this).parent());
	location.href="toCertificate/"+custom.customCourseId;
});
$(".box-body").find(".audit-score").unbind("click").bind("click",function() {
	var custom = buildCustom($(this).parent());
	location.href="toAuditScore/"+custom.customCourseId;
});
function buildCustom(t) {
	...
	var orgName = tr.find(".orgName").html();
