		...
		<div class="box-body">
			<div class="filter-tabs filter-tabs2 clearfix">
				<ul class="list-unstyled">
					<li lang=":input[name='search_EQ_status']" <c:if test="${empty param['search_EQ_status']}">class="actived"</c:if>>全部（${stayOpen + submitPropose + confirmPropose + submitThesis}）</li>
					...
					<li value="4" role=":input[name='search_EQ_status']" <c:if test="${param['search_EQ_status'] == 4 }">class="actived"</c:if>>初稿待定稿评分（${submitThesis}）</li>
				</ul>
			</div>
			<div>
								...
								<c:forEach items="${pageInfo.content}" var="entity">
									<c:if test="${not empty entity}">
						            	<tr>
						            		...
						            		<td>
						            			<c:if test="${isBtnView}">
						            				<a href="${ctx}/thesisApply/view?thesisPlanId=${entity.thesisPlanId}&studentId=${entity.studentId}&isTeacher=true" class="operion-item" data-toggle="tooltip" title="查看详情" data-role="set"><i class="fa fa-fw fa-view-more"></i></a>
						            			</c:if>
						            		</td>
						            	</tr>
		            				</c:if>
