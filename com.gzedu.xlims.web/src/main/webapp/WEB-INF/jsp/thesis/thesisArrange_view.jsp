...
<form id="theform" action="${ctx }/thesisArrange/update" method="post">
<input type="hidden" name="arrangeId" value="${entity.arrangeId}">
<div class="box no-border no-shadow">
	<div class="box-header with-border">
      <h3 class="box-title">设置毕业论文安排</h3>
    </div>
    <div class="box-body scroll-box">
		<table class="table-gray-th">
			...
			<tr>
				<th class="text-center"><small class="text-red">*</small>开题报告示例</th>
				<td>
					<input type="hidden" id="exampleUrl" name="exampleUrl" class="form-control" value="${entity.exampleUrl}"/>
					<input type="hidden" id="exampleName" name="exampleName" class="form-control" value="${entity.exampleName}"/>
					<a id="exampleA" href="${entity.exampleUrl}" target="_blank">${entity.exampleName}</a>
					<button class="btn btn-default btn-sm" name="headPortrait" type="button" onclick="javascript:uploadFile('exampleName','exampleUrl','doc|docx|wps', null, uploadCallback)">上传</button>
				</td>
			</tr>
			<tr>
				...
				<td>
					${entity.applyNum}人
					<small class="gray9">
						已分配  <b class="text-orange guideNum">${fn:length(entity.gjtThesisAdvisers1)}</b> 个老师，已分配指导 <b class="text-orange distributionNum"></b> 人，剩余 <b class="text-orange remainderNum"></b> 人待分配
			...
			<tr>
				<th class="text-center">
					<small class="text-red">*</small>论文指导老师
					<div class="margin_t5">
				...
				<td>
					<div class="clearfix" style="margin:0 -5px;" data-box="1">
						<c:forEach items="${entity.gjtThesisAdvisers1}" var="adviser">
				                    ...
				                    <div class="media no-margin">
				                    	...
				                    	<div class="media-body media-middle">
				                    		...
				                    		<div class="margin_t10">
				                    			...
				                    			人
				                    			<input type="hidden" name="advisers1" value="${adviser.teacherId}">
				                    		</div>
				                    	</div>
				                    </div>
				                </div>
...
;(function(){
	...
	var postForm=$theform.Validform({
	  ...
	  beforeSubmit:function(curform){
		  ...
		  if($('input[name="advisers1"]').length == 0) {
	            alert('请添加论文指导老师');
	            return false;
	      }
...
.on('click','[data-role="add-person-1"]',function(event) {
	var advisers1 = '';
	$('input[name="advisers1"]').each(function(index, el) {
		advisers1 = advisers1 + el.value + ',';
	});
	if (advisers1 != '') {
		advisers1 = advisers1.substring(0, advisers1.length - 1);
	}
	...
	var _this=this;
	  ...
	  content: 'url:${ctx}/thesisArrange/choiceGuideTeacher?arrangeId='+arrangeId+"&teacherIds="+advisers1
	});
})
//添加论文答辩老师
...
function uploadCallback() {
	$("#exampleA").attr("href", $("#exampleUrl").val());
