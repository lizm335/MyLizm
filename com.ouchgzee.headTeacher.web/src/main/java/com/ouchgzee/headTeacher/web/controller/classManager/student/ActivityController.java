 ...
 */
@Controller
@RequestMapping("/home/class/activity")
public class ActivityController extends BaseController {
	...
	public Feedback doCreateActivity(ServletRequest request, RedirectAttributes redirectAttributes,
			HttpSession session) {
		Feedback feedback = new Feedback(true, "创建成功", "正在跳转！");

		String beginTimeStr = request.getParameter("beginTime");
		String endTimeStr = request.getParameter("endTime");
		// String[] activityPictures =
		// request.getParameterValues("activityPicture");
		...
		if ("2".equals(isFreeStr) && StringUtils.isNotBlank(chargeMoneyStr)) {
	...
	@ResponseBody
	public Feedback addImg(HttpServletRequest request) {
		Feedback fb = new Feedback(true, "上传成功");
		String imgBase64 = request.getParameter("imgBase");
		try {
			String tmpFolderPath = request.getSession().getServletContext().getRealPath("") + File.separator + "tmp"
					+ File.separator + DateFormatUtils.ISO_DATE_FORMAT.format(Calendar.getInstance());
			String signPath = EEFileUploadUtil.uploadImageBase64ToUrl(imgBase64, tmpFolderPath);
			fb = new Feedback(true, signPath);
		} catch (Exception e) {
			fb = new Feedback(false, "图片黏贴失败，发生未知异常");
