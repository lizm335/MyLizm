package com.ouchgzee.headTeacher.web.aop;
...
import com.gzedu.xlims.constants.syslog.executor.SysLogExecutor;
...
public class SysLogAspect {
	...
	@Pointcut("@annotation(com.gzedu.xlims.common.annotation.SysLog) || execution(* com.ouchgzee.headTeacher..controller..create(..)) || execution(* com.ouchgzee.headTeacher..controller..update(..)) || execution(* com.ouchgzee.headTeacher..controller..delete*(..))")
	...
	public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			...
			slt.setRequestParams(request.getParameterMap());
			slt.getSysLog().setMethod(request.getServletPath());
			slt.getSysLog().setTime(time);
			slt.getSysLog().setError(error);
			// 设置IP地址
			slt.getSysLog().setIp(IPUtils.getIpAddr(request));
			slt.getSysLog().setPlatfromType(3); // 班主任平台
			// 放入线程池中执行
