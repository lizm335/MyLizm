...
<body>
	<header class="header">
		<div class="login-center padding_t20 padding_b20">
			<div class="logo ">
				<c:if test="${not empty sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginHeadLogo }">
					<img src="${sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginHeadLogo }" >
				</c:if>
				<c:if test="${empty sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginHeadLogo }">
	...
	<c:choose>
		<c:when test="${not empty sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginBackground }">
			<section class="content" style="background-image:url(${sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginBackground})">
		</c:when>
		<c:otherwise>
				...
				<div class="inner">
					<form id="loginForm" name="loginForm" action="${ctx}/login" method="post">
						...
						<div class="login-wrap">
							<c:if test="${not empty sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginTitle }">
								<h1 class="login-title">${sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginTitle }</h1>
							</c:if>
							<c:if test="${empty sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginTitle }">
								<h1 class="login-title">班主任登录</h1>
							</c:if>
							...
							<div class="input-group clearfix inline-block vertical-middle" style="width: 155px;">
								...
								<div class="input-control">
									<input id="rCode" name="rCode" class="form-control" placeholder="验证码" type="text" style="width: 90px!ie;">
								</div>
							</div>
							...
							<div class="clearfix margin_t15">
								<div class="dropdown fr">
									<c:if test="${not empty sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.qcodePic }">
										<div class="dropdown-toggle">
											<i class="icon icon-tel vertical-middle margin_r5"></i> <u>App下载</u>
										</div>
										<div class="dropdown-menu">
											<img src="${sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.qcodePic }" class="vertical-middle margin_r10" style="width: 126px;">
	...
	<footer class="footer">
		<div class="login-center">
			<div align="center" class="padding_t20 padding_b15">
				<c:if test="${not empty sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginFooterCopyright }">
					${sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginFooterCopyright }
				</c:if>
				<c:if test="${ empty sessionScope.LOGIN_NAMESPACE_COPYRIGHT_HEAD.loginFooterCopyright }">
