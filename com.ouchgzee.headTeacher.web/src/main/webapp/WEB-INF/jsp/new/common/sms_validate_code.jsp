...
<body>
    <form id="exportValidateForm" name="exportValidateForm" action="doSmsValidateCode" method="post">
        <input type="hidden" name="smsCodeMark" value="${smsCodeMark}" />
            ...
            <div class="box-body pos-rel overlay-wrapper">
                ...
                <div class="text-center pad-t30 margin_t10 col-xs-8 col-xs-offset-2 pos-rel">
                    <div class="input-group input-group-md">
                        <input type="text" name="code" class="form-control tel-vercode" placeholder="请输入手机验证码">
                        <span class="input-group-btn">
                            <button class="btn btn-primary send-sms" type="button" data-role="get-ver-code">获取验证码</button>
                        </span>
                    </div>
    ...
    <script type="text/javascript">
        ...
        $('[data-role="export"]').click(function(event) {
            window.parent.smsCallback(); // 回调函数
        ...
        $('[data-role="get-ver-code"]').click(function(event){
            ...
            $.ajax({
                type: "POST",
                url: "${ctx}/home/common/sendSmsCode",
                data: {},
                dataType: "json",
                async: true,	// 是否异步
        ...
        $(".tel-vercode").keyup(function(event) {
            ...
            if($.trim(this.value)==""){
                ...
                if(this.value.length!=6 || isNaN(this.value))return;//6位数才查询
                $verTips.show();
                $.ajax({
                    type: "POST",
                    url: "${ctx}/home/common/doSmsValidateCode",
                    data: {code: $(".tel-vercode").val()},
                    dataType: "json",
                    async: true,	// 是否异步
                    success: function(data){
