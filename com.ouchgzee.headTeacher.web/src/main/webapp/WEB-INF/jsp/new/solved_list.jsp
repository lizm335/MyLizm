										...
										<form id="listForm" action="${ctx }/home/manyFeedSubject">
											...
											<div class="clearfix faq-list-bar">
												...
												<div class="filter-group-box">
													<div class="btn-group ">
														<input type="hidden" value="${isSolve }" name="isSolve"> 
														<a data-id="" data-role=":input[name='isSolve']"
															class="btn btn-default <c:if test="${empty isSolve }">active</c:if>"> 全部已解答(${noAffirmSolve+yesAffirmSolve})
														</a> 
														<a data-id="0" data-role=":input[name='isSolve']" 
															class="btn btn-default <c:if test="${isSolve  eq '0'}">active</c:if>"> 未确认解决的(${noAffirmSolve })
														</a> 
														<a data-id="1" data-role=":input[name='isSolve']" 
															class="btn btn-default <c:if test="${isSolve  eq '1'}">active</c:if>">已确认解决 (${yesAffirmSolve })
														</a>
													</div>
												</div>
											</div>
		...
		$('[data-role="add-faq"]').click(function(event) {
			var $id=$(this).data('id');
			  $.mydialog({
			        ...
			        height:350,
			        zIndex:11000,
			        content: 'url:'+ctx+'/home/class/newFeedback/settingOften?subjectId='+$id
		      });
		});
