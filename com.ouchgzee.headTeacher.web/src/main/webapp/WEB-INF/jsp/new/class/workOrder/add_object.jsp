    ...
    if($ck.length>0){
        var htmlTemp=['<li>',
                        ...
                        '<i class="fa fa-fw fa-remove" data-toggle="tooltip" title="删除"></i>',
                         '<input type="hidden" name="assignPersons" value="{2}">',
                      '</li>'].join('');
        var result=[];
        ...
        if(json.frame=='parent'){
            parent.$(json.acceptBox).html(result.join(''));
        }
        else{
            parent.$('iframe[name="Iframe-'+json.frame+'"]').contents().find(json.acceptBox).html(result.join(''));
        }
    }
    parent.$.closeDialog(frameElement.api)
});
