									...
									<c:if test="${type eq '0' }">
										<td width="150">
											<c:if test="${info.isState ne '2' }"> 
												...
												<a href="${ctx }/home/class/workOrder/update?id=${info.id }" class="btn btn-default min-width-90px margin5"  title="编辑" >
												编辑
											</a>
											</c:if>
		...
		$('.addComment').click(function(){
			...
			if(comment.length<1){
				alert('讨论内容不能为空！');
				 ...
				 $.get('${ctx}/home/class/workOrder/createComment',{id:workOrderId,content:comment},function(data){
		...
		$('[data-role="confirm-sure"]').click(function(){
			 $.alertDialog({
	               ...
	               ok:function(){//“确定”按钮的回调方法
	            	   $.get('${ctx}/home/class/workOrder/updateState',{id:workOrderId,state:1},function(data){
	           	 		if(data.successful==false){
	           	 			alert(data.message);
	           	 		}else{
	           	 			window.location.href=window.location.href;
		...
		$('[data-role="confirm-close"]').click(function(){
			...
			if($state=='0'){
			 ...
			 $.alertDialog({
	               ...
	               zIndex:11000,
	               ok:function(){//“确定”按钮的回调方法
	            	   $.get('${ctx}/home/class/workOrder/updateState',{id:workOrderId,state:$state},function(data){
	           	 		if(data.successful==false){
	           	 			alert(data.message);
	           	 		}else{
	           	 			window.location.href=window.location.href;
		...
		$('[data-role="btn-delete"]').click(function(){
			 var $id=$(this).data('id');
	           $.alertDialog({
	               ...
	               zIndex:11000,
	               ok:function(){//“确定”按钮的回调方法
	            	   $.get("${ctx}/home/class/workOrder/deleteComment",{id:$id},function(data){
		               		if(data.successful==false){
								alert(data.message);
							}else{
								window.location.href=window.location.href;
