										...
										<form id="listForm" action="${ctx }/home/manyFeedSubject">
											...
											<div class="clearfix faq-list-bar">
												...
												<div class="filter-group-box">
													<div class="btn-group ">
														<input type="hidden" value="${isAsk }" name="isAsk"> 
														<a data-id="" data-role=":input[name='isAsk']"
															class="btn btn-default <c:if test="${empty isAsk }">active</c:if>"> 全部问题(${forwardCount })
														</a> 
														<a data-id="N" data-role=":input[name='isAsk']" 
															class="btn btn-default <c:if test="${isAsk  eq 'N'}">active</c:if>">待解答(${forwardNoSolveCount })
														</a> 
														<a data-id="YN" data-role=":input[name='isAsk']" 
															class="btn btn-default <c:if test="${isAsk  eq 'YN'}">active</c:if>"> 已解答，未确认解决(${forwardYnSolve })
														</a> 
														<a data-id="YY" data-role=":input[name='isAsk']" 
															class="btn btn-default <c:if test="${isAsk  eq 'YY'}">active</c:if>">已解答，已确认解决 (${forwardYYSolve })
														</a>
													</div>
												</div>
											</div>
