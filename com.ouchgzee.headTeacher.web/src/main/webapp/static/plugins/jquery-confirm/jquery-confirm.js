...
(function ($) {
    ...
    Jconfirm.prototype = {
        ...
        _hash: function () {
			var result='';
			if(window.btoa){
				btoa((encodeURIComponent(this.$content.html())));
			}
			else{
				this._encode64(this.$content.html());
			}
        ...
        setContent: function (string) {
            ...
            } else if(this.$content.children("iframe").length<=0){
                this.$content.html(this.content);
        ...
        _getContent: function (string) {
            ...
            if (!this.content) { // if the content is falsy
                ...
                if (this.content.substr(0, 4).toLowerCase() === 'url:') {
					...
					var iframName=$.trim(that.iframeId);
					if(iframName !== ''){
						var $iframe=$("<iframe>");
						if(iframName=='auto'){
							iframName='iframe-'+Math.round(Math.random() * 99999);
						}
						$iframe.prop({
						  id:iframName,
						  name:iframName,
    ...
    jconfirm.pluginDefaults = {
        ...
        contentHeight:false,
		iframeId:'',//如果引用的是html单页面，请务必使用此属性；如果想自动生成随机Id，可设置为'auto';
        icon: '',    
        opacity: 0.2,
        confirmButton: 'Okay',
        cancelButton: 'Close',
