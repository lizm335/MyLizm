...
/**
 * @author lianghuahuang Feb 26, 2009 4:41:27 PM
 */
public class EncryptUtil {
	...
	public static String decrypt(String encryptedStr) { // 接收人事局数据，并解密
		...
		for (i = 0; i < arrMsg.length; i++) {
			mKey = sKey.charAt(i) + "";
			mData = Integer.parseInt(arrMsg[i].toUpperCase(), 16)
					^ mKey.hashCode();
			mChar = (char) mData;
		...
		try {
			if (sMsg != null) {
				...
				for (i = 0; i < sMsg.length(); i++) {
					mKey = sKey.charAt(i) + "";
					sReturn = sReturn
							+ (Integer.toHexString(sMsg.charAt(i)
	...
	public static String encryptInfo(String encryptStr) {
		...
		for (int i = 0; i < encryptStr.length(); i++) {
			ascCode = (int) encryptStr.charAt(i);
			ascCodeStr = String.valueOf(ascCode);
			charAscCodeSum = 0;
			firstCharAscCode = 0;
			codeStr = "";
			charAscCodeCharArray = ascCodeStr.toCharArray();
			for (int j = 0; j < charAscCodeCharArray.length; j++) {
				charAscCodeSum += Integer.parseInt(String
						.valueOf(charAscCodeCharArray[j]));
				subCharAscCode = 65 + Integer.parseInt(String
						.valueOf(charAscCodeCharArray[j]));
	...
	public static String getEncryptFtpInfo(String ftpIp, String ftpUser,
			String ftpPwd) {
		String encryptedFtpStr = "";
		encryptedFtpStr = "'" + encryptInfo(ftpIp) + "','"
				+ encryptInfo(ftpUser) + "','" + encryptInfo(ftpPwd) + "'";
		return encryptedFtpStr;
	}

	public static String getEncryptFtpInfo() {
		String encryptedFtpStr = "";
		String ftpIp = "", ftpUser = "", ftpPwd = "";
		ftpIp = CommonConfig.getProperty("ftp.server");
		ftpUser = CommonConfig.getProperty("ftp.username");
		ftpPwd = CommonConfig.getProperty("ftp.password");
		encryptedFtpStr = "'" + encryptInfo(ftpIp) + "','"
				+ encryptInfo(ftpUser) + "','" + encryptInfo(ftpPwd) + "'";
		return encryptedFtpStr;
	}
